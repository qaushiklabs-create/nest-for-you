...
.hero-right-text p {
  ...
  text-shadow: 0 4px 18px rgba(0, 0, 0, 0.6);  /* Subtle shadow for better visibility */
  max-width: 100%;  /* Ensure it stays within the container */
  text-align: center;  /* Keep the text centered */
  word-wrap: break-word; /* Ensure text wraps nicely */
    margin-top: 40px; /* Adjust this value to move the text lower */
}
...
@media (max-width: 520px){
  ...
  .hero-info-text {
    font-size: 18px;  /* Slightly bigger font size for small mobile */
    line-height: 1.6;
  }
...
<main class="main">
  <section id="hero" class="hero section dark-background">
    <img src="/nestforyou/assets/img/hero-bg.jpg" alt="Nestforyou real estate in Gurgaon">
  ...
  <div class="hero-right-text">
    <p>
      Find verified properties and connect with trusted real estate agents in Gurgaon.
    </p>
  </div>
      </div>
    </div>
 ...
 searchForm.addEventListener("submit", function(e){
  ...
  if (aptSlug) {
    window.location.href =
      "/real-estate-agent/" + citySlug + "/" + sectorSlug + "/" + aptSlug;
  } else {
    window.location.href =
      "/real-estate-agent/" + citySlug + "/" + sectorSlug;
  }
});
...
function scrollForDropdown(selectEl){
  ...
  selectEl.addEventListener("focus", function(){
    ...
    // If space below is not enough, scroll page UP
    if(spaceBelow < 300){
      ...
      window.scrollTo({
        top: targetY,
        behavior: "instant"   // IMPORTANT: no smooth
      });
    }
  });
}
